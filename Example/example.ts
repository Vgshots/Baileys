...
const startSock = async () => {
  ...
  sock.ev.process(
    ...
    async (events) => {
      ...
      if (events.call && events.call.length > 0) {
        console.log("Received call events:", events.call);
        const call = events.call[0]; // Access the first call event

        try {
          // Ensure that callId and callFrom are both provided
          if (!call.id || !call.from) {
            console.error("Error: Missing callId or callFrom for call:", call);
            return; // Exit the function or continue with the rest of your code
          }
          await sock.rejectCall(call.id, call.from);
          console.log("Rejected incoming call from:", call.from);

          //await sendMessageWTyping({ text: 'Your call has been rejected.' }, call.from);

          console.log("Message sent to rejected caller:", call.from);
        } catch (error) {
